...
describe("TsConfigHelper", function () {
  ...
  describe("#findAndParseTsConfig", function () {
    it("should find and parse a standard tsconfig.json", async function () {
      const result = await TsConfigHelper.findAndParseTsConfig(
        simpleProjectPath
      );

      assert.ok(result, "Should return a ProcessedTsConfig object");
      assert.strictEqual(
        typeof result.absoluteBaseUrl,
        "string",
        "Should have absoluteBaseUrl"
      );
      assert.strictEqual(
        typeof result.paths,
        "object",
      ...
      assert.strictEqual(
        typeof result.sourceFilePath,
        "string",
        "Should have sourceFilePath"
      );
      ...
      assert.strictEqual(
        ...
        simpleProjectPath,
        "BaseUrl should be project root"
      );
      ...
      assert.ok(
        result.paths["@shared/*"],
        "Should have @shared/* path mapping"
      );
    ...
    it("should find and parse tsconfig.base.json for Nx projects", async function () {
      const result = await TsConfigHelper.findAndParseTsConfig(nxProjectPath);

      assert.ok(
        result,
        "Should return a ProcessedTsConfig object for Nx project"
      );
      assert.strictEqual(
        result.absoluteBaseUrl,
        nxProjectPath,
      ...
      assert.ok(
        result.paths["@myorg/shared-ui"],
        "Should have @myorg/shared-ui path mapping"
      );
      assert.ok(
        result.paths["@myorg/shared-data"],
        "Should have @myorg/shared-data path mapping"
      );
      assert.ok(
        result.paths["@myorg/feature-auth"],
        "Should have @myorg/feature-auth path mapping"
      );
      assert.ok(
        result.paths["@myorg/core"],
        "Should have @myorg/core path mapping"
      );

      // Verify Nx path arrays
      assert.deepStrictEqual(result.paths["@myorg/shared-ui"], [
        "libs/shared/ui/src/index.ts",
      ]);
      assert.deepStrictEqual(result.paths["@myorg/shared-data"], [
        "libs/shared/data/src/index.ts",
      ]);
    });

    it("should handle complex tsconfig with multiple path mappings", async function () {
      const result = await TsConfigHelper.findAndParseTsConfig(
        complexProjectPath
      );
      ...
      assert.ok(
        result,
        "Should return a ProcessedTsConfig object for complex project"
      );
      ...
      assert.strictEqual(
        result.absoluteBaseUrl,
        expectedBaseUrl,
        "BaseUrl should be src subdirectory"
      );
      ...
      assert.ok(result.paths["@app/*"], "Should have @app/* path mapping");
      assert.ok(
        result.paths["@components/*"],
        "Should have @components/* path mapping"
      );
      assert.ok(
        result.paths["@services/*"],
        "Should have @services/* path mapping"
      );
      ...
      assert.deepStrictEqual(result.paths["@components/*"], [
        "app/components/*",
      ]);
    ...
    it("should return null for projects without tsconfig", async function () {
      const result = await TsConfigHelper.findAndParseTsConfig(
        noTsconfigProjectPath
      );

      assert.strictEqual(
        result,
        null,
        "Should return null when no tsconfig is found"
      );
    });

    it("should cache results for the same project", async function () {
      // First call
      const result1 = await TsConfigHelper.findAndParseTsConfig(
        simpleProjectPath
      );

      // Second call should return cached result
      const result2 = await TsConfigHelper.findAndParseTsConfig(
        simpleProjectPath
      );
    ...
    it("should handle invalid project paths gracefully", async function () {
      ...
      assert.strictEqual(
        result,
        null,
        "Should return null for invalid project paths"
      );
    });
  });
  ...
  describe("#resolveImportPath", function () {
    ...
    before(async function () {
      ...
      simpleProjectConfig = (await TsConfigHelper.findAndParseTsConfig(
        simpleProjectPath
      )) as ProcessedTsConfig;
      complexProjectConfig = (await TsConfigHelper.findAndParseTsConfig(
        complexProjectPath
      )) as ProcessedTsConfig;
      ...
      assert.ok(
        ...
        "Complex project config should be loaded"
      );
    });

    describe("Alias Resolution", function () {
      it("should resolve @app/* aliases correctly", async function () {
        ...
        const result = await TsConfigHelper.resolveImportPath(
          targetPath,
          currentFile,
          simpleProjectPath
        );
        ...
        assert.strictEqual(
          ...
          "Should resolve to @app alias"
        );
      });

      it("should resolve @shared/* aliases correctly", async function () {
        // Ensure tsconfig is loaded first
        await TsConfigHelper.findAndParseTsConfig(simpleProjectPath);
        const targetPath = path.join(
          simpleProjectPath,
        ...
        assert.strictEqual(
          result,
          "@shared/utils/helper",
          "Should resolve to @shared alias"
        );
      });

      it("should resolve barrel imports (@core) correctly", async function () {
        // Ensure tsconfig is loaded first
        await TsConfigHelper.findAndParseTsConfig(simpleProjectPath);
        ...
        assert.strictEqual(
          result,
          "@core",
          "Should resolve to @core barrel import"
        );
      });

      it("should handle complex nested aliases", async function () {
        // Ensure tsconfig is loaded first
        ...
        assert.strictEqual(
          ...
          "Should resolve to @components alias"
        );
      });

      it("should resolve paths outside src using @lib/* alias", async function () {
        // Ensure tsconfig is loaded first
        await TsConfigHelper.findAndParseTsConfig(complexProjectPath);
        const targetPath = path.join(
          complexProjectPath,
        ...
        assert.strictEqual(
          result,
          "@lib/shared/utils",
          "Should resolve to @lib alias for external libs"
        );
      });

      it("should prefer more specific aliases over general ones", async function () {
        // Ensure tsconfig is loaded first
        await TsConfigHelper.findAndParseTsConfig(complexProjectPath);
        ...
        assert.strictEqual(
          result,
          "@components/button",
          "Should prefer @components over @app"
        );
      });

      it("should handle case-insensitive path matching", async function () {
        // Ensure tsconfig is loaded first
        await TsConfigHelper.findAndParseTsConfig(simpleProjectPath);
    ...
    describe("Relative Path Fallback", function () {
      it("should fall back to relative paths when no alias matches", async function () {
        const targetPath = path.join(
          simpleProjectPath,
          "src",
          "assets",
      ...
      it("should handle same directory relative imports", async function () {
        const targetPath = path.join(
          simpleProjectPath,
          "src",
          "app",
      ...
      it("should handle parent directory relative imports", async function () {
        const targetPath = path.join(
          simpleProjectPath,
          "src",
          "app",
    ...
    describe("Edge Cases", function () {
      it("should handle files outside project root", async function () {
        ...
        assert.strictEqual(
          result,
          "/some/external/path/module",
          "Should return absolute path for external files"
        );
      });

      it("should handle current file outside project root", async function () {
        const targetPath = path.join(
          simpleProjectPath,
          "src",
          "app",
        ...
        assert.ok(
          result.includes("test-component"),
          "Should include target component name"
        );
      });

      it("should handle empty or invalid paths gracefully", async function () {
        const currentFile = path.join(
          simpleProjectPath,
          "src",
          "app",
        ...
        const result1 = await TsConfigHelper.resolveImportPath(
          "",
          currentFile,
          simpleProjectPath
        );
        ...
        assert.ok(
          ...
          "Should return string for invalid project root"
        );
      });

      it("should prefer aliases over relative paths when both are possible", async function () {
        // Ensure tsconfig is loaded first
        await TsConfigHelper.findAndParseTsConfig(simpleProjectPath);
        ...
        assert.strictEqual(
          result,
          "@app/utils/helper",
          "Should prefer alias over relative path"
        );
      });
    });

    describe("Cache Management", function () {
      it("should use cached trie for subsequent resolutions", async function () {
        // Ensure tsconfig is loaded first
        await TsConfigHelper.findAndParseTsConfig(simpleProjectPath);
        ...
        assert.strictEqual(
          result1,
          "@app/components/comp1",
          "First resolution should work"
        );
        assert.strictEqual(
          result2,
          "@app/services/service1",
          "Second resolution should use cached trie"
        );
      });
      ...
      it("should clear cache correctly", async function () {
        ...
        assert.strictEqual(
          result2,
          "@app/components/test",
          "Should resolve after cache clear"
        );
      });

      it("should handle cache clear for all projects", async function () {
        // Ensure tsconfig is loaded first
        await TsConfigHelper.findAndParseTsConfig(simpleProjectPath);
        await TsConfigHelper.findAndParseTsConfig(complexProjectPath);
        ...
        const currentFile1 = path.join(
          simpleProjectPath,
          "src",
          "app",
          "home.ts"
        );
        const currentFile2 = path.join(
          complexProjectPath,
          "src",
          "app",
          "home.ts"
        ...
        await TsConfigHelper.resolveImportPath(
          targetPath1,
          currentFile1,
          simpleProjectPath
        );
        ...
        assert.strictEqual(
          result1,
          "@app/test1",
          "Simple project should work after global cache clear"
        );
        assert.strictEqual(
          result2,
          "@app/test2",
          "Complex project should work after global cache clear"
        );
      });
    });
  });

  describe("PathAliasTrie (Internal)", function () {
    it("should handle wildcard vs barrel alias precedence correctly", async function () {
      // This tests the internal trie logic for handling different alias types
      const config = (await TsConfigHelper.findAndParseTsConfig(
        simpleProjectPath
      )) as ProcessedTsConfig;
      ...
      const wildcardTarget = path.join(
        simpleProjectPath,
        "src",
        "app",
        "components",
      ...
      const barrelResult = await TsConfigHelper.resolveImportPath(
        barrelTarget,
        currentFile,
        simpleProjectPath
      );
      const wildcardResult = await TsConfigHelper.resolveImportPath(
        wildcardTarget,
        currentFile,
        simpleProjectPath
      );
      ...
      assert.strictEqual(
        barrelResult,
        "@core",
        "Should use barrel import for exact match"
      );
      assert.strictEqual(
        wildcardResult,
        "@app/components/test",
        "Should use wildcard for path extension"
      );
    });

    it("should handle longest prefix matching correctly", async function () {
      ...
      const componentResult = await TsConfigHelper.resolveImportPath(
        componentTarget,
        currentFile,
        complexProjectPath
      );
      const serviceResult = await TsConfigHelper.resolveImportPath(
        serviceTarget,
        currentFile,
        complexProjectPath
      );
      ...
      assert.strictEqual(
        serviceResult,
        "@services/data",
        "Should prefer @services over @app"
      );
    });
  });

  describe("Error Handling", function () {
    it("should handle malformed tsconfig.json gracefully", async function () {
      ...
      try {
        ...
        assert.strictEqual(
          result,
          null,
          "Should return null for malformed tsconfig"
        );
      } finally {
    ...
    it("should handle missing compilerOptions gracefully", async function () {
      // Create a temporary tsconfig without compilerOptions
      const tempProjectPath = path.join(
        fixturesPath,
        "temp-no-compiler-options"
      );
      const tempTsconfigPath = path.join(tempProjectPath, "tsconfig.json");
      ...
      try {
        ...
        assert.strictEqual(
          typeof result.absoluteBaseUrl,
          "string",
          "Should have default baseUrl"
        );
        assert.deepStrictEqual(
          result.paths,
          {},
          "Should have empty paths object"
        );
      } finally {
        // Clean up in finally block to ensure cleanup even if test fails
        try {
    ...
    it("should handle permission errors gracefully", async function () {
      ...
      assert.strictEqual(
        result,
        null,
        "Should return null for inaccessible paths"
      );
    });
  });
});
